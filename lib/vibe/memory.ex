defmodule Vibe.Memory do
  ...
  alias Vibe.Storage
  alias Vibe.Storage.{Schema.Memory, Search}
  ...
  def search(query, opts \\ []) when is_binary(query) do
    ...
    query
    |> Vibe.Storage.Search.memories(memory_scopes: scopes, limit: limit)
  ...
  def remove(scope, id) when is_binary(id) do
    ...
    |> case do
      ...
      {_count, _rows} ->
        Vibe.Storage.FTS.remove_memory(id)
        :ok
    end
  end
  ...
  defp insert(scope, text) do
    ...
    |> case do
      {:ok, memory} ->
        Vibe.Storage.FTS.index_memory(memory)
