defmodule Vibe.CLI.Storage do
  ...
  def command(["migrate"], opts) do
    Output.print(run(fn -> Vibe.Storage.migrate!() end), opts)
  end

  def command(["status"], opts) do
    Output.print(run(&Vibe.Storage.status/0), opts)
  end

  def command(["checkpoint"], opts) do
    Output.print(run(fn -> Vibe.Storage.checkpoint!() end), opts)
  end

  def command(["vacuum"], opts) do
    Output.print(run(fn -> Vibe.Storage.vacuum!() end), opts)
  end

  def command(["fts", "status"], opts) do
    Output.print(run(&Vibe.Storage.FTS.status/0), opts)
  end

  def command(["fts", "rebuild"], opts) do
    Output.print(run(fn -> Vibe.Storage.FTS.rebuild(progress: progress_fun(opts)) end), opts)
  end

  def command(["fts", "optimize"], opts) do
    Output.print(run(fn -> Vibe.Storage.FTS.optimize() end), opts)
  end
  ...
  def command(["import", source, path], opts) do
    Output.print(Vibe.Storage.Import.import_path(source, path, import_opts(opts)), opts)
  end

  def command(_args, _opts) do
    Output.error(
      "Usage: vibe storage migrate|status|checkpoint|vacuum|fts status|fts rebuild|fts optimize|search <query>|import pi <path> [--no-fts] [--rebuild-fts] [--batch-size N]"
    )
