defmodule Vibe.CLI.Runner do
  @moduledoc "Prompt execution for print mode and TUI startup."
  alias Vibe.CLI.{Logging, Output}
  ...
  def ask("", _opts) do
    Output.error("No prompt provided. Run `mix vibe --help` for usage.")
      ...
      Logging.with_session_log(session_id, fn ->
        if opts[:direct] do
          ...
          case direct_prompt(prompt, opts) do
            ...
            prompt ->
              if stream?(opts) do
                Vibe.Model.Direct.stream(prompt, llm_opts)
              else
          ...
          case agent_prompt(prompt, opts) do
            ...
            prompt ->
              with {:ok, pid} <- Vibe.start_link(agent_opts(opts)) do
                Vibe.ask(pid, prompt,
  ...
  defp direct_ask_fun(opts), do: Keyword.get(opts, :direct_ask_fun, &Vibe.Model.Direct.ask/2)

  defp direct_prompt(prompt, opts) do
    case process_file_args(opts) do
      {:ok, nil} ->
        Vibe.Prompt.Attachments.expand(prompt, root: File.cwd!())
  ...
  defp process_file_args(opts) do
    ...
    if file_args == [] do
      ...
      case Vibe.Prompt.Attachments.process_file_args(file_args, root: File.cwd!()) do
  ...
  def tui(opts, runtime_extra \\ []) do
    ...
    case Logging.with_session_log(runtime_opts[:session_id], fn ->
           Vibe.TUI.Runtime.run(runtime_opts)
         end) do
      :ok ->
        :ok

      {:error, reason} ->
        Output.error("Cannot start Vibe TUI: #{reason}")
        {:error, reason}
    end
  end
  ...
  if Vibe.Debug.enabled?() do
    defp trace_print_delta(type) do
      fn text ->
        Vibe.Agent.Streaming.Trace.record(:print_delta, %{chunk_type: type, text: text})
      end
    end
  else
  ...
  def session_id(opts), do: opts[:session] || Vibe.Session.Store.new_id()

  if Vibe.Debug.enabled?() do
    defp trace_dir(opts), do: opts[:trace_tui] || System.get_env("VIBE_TUI_TRACE_DIR")
  else
