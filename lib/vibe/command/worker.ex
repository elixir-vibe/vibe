defmodule Vibe.Command.Worker do
  @moduledoc "Task-based command execution worker."
  use GenServer

  alias Vibe.Command.{Job, Result}
  ...
  def init(opts) do
    ...
    env = opts |> Keyword.get(:env, []) |> Vibe.Env.to_charlist_pairs()
  ...
  defp default_output_path(id), do: Path.join([Vibe.Paths.home(), "commands", id <> ".log"])
