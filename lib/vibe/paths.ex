defmodule Vibe.Paths do
  @moduledoc "Canonical filesystem paths for Vibe runtime data."
  @spec home() :: String.t()
  def home do
    (System.get_env("VIBE_HOME") || Application.get_env(:vibe, :home_dir, "~/.vibe"))
    |> Path.expand()
  end

  @spec database() :: String.t()
  def database do
    System.get_env("VIBE_DB_PATH") || Application.get_env(:vibe, :database_path) ||
      default_database()
  end
  ...
  defp build_default_database do
    case Application.get_env(:vibe, :env) do
      :test -> Path.join(System.tmp_dir!(), "vibe-test-#{System.unique_integer([:positive])}.db")
      _env -> Path.join(home(), "vibe.db")
    end
  end
  ...
  def sessions_dir do
    System.get_env("VIBE_SESSION_DIR") || Application.get_env(:vibe, :session_dir) ||
      Path.join(home(), "sessions")
  end

  @spec telemetry_dir() :: String.t()
  def telemetry_dir do
    System.get_env("VIBE_TELEMETRY_DIR") || Application.get_env(:vibe, :telemetry_dir) ||
  ...
  def memory_dir do
    System.get_env("VIBE_MEMORY_DIR") || Application.get_env(:vibe, :memory_dir) ||
      Path.join(home(), "memory")
  end

  @spec agent_profiles() :: String.t()
  def agent_profiles do
    System.get_env("VIBE_AGENT_PROFILES") || Application.get_env(:vibe, :agent_profiles_file) ||
      Path.join(home(), "agent-profiles.toml")
  end

  @spec subagents_dir() :: String.t()
  def subagents_dir do
    System.get_env("VIBE_SUBAGENTS_DIR") || Application.get_env(:vibe, :subagents_dir) ||
