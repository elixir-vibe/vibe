defmodule Vibe.TUI.Widgets.Tools.FileMutation do
  @moduledoc "TUI tool widget: shared file mutation display."
  alias Vibe.TUI.{Syntax, TextTruncation, Theme, ToolWidget, Widget}
  ...
  defp language(%{path: path}) when is_binary(path), do: Vibe.Files.language(path)
  defp language(%{change: %{path: path}}) when is_binary(path), do: Vibe.Files.language(path)
