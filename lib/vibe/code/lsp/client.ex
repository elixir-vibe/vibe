defmodule Vibe.Code.LSP.Client do
  ...
  def ensure_started(cwd) do
    ...
    case Registry.lookup(Vibe.Registry, {:lsp, cwd}) do
      [{pid, _}] ->
        {:ok, pid}
  ...
  def init(opts) do
    ...
    Registry.register(Vibe.Registry, {:lsp, cwd}, [])
  ...
  defp expert_command do
    cond do
      ...
      true ->
        raise "Expert executable not found. Install expert or set VIBE_EXPERT=/path/to/expert"
    end
  end

  defp expert_args(_command), do: ["--stdio"]

  defp via(cwd), do: {:via, Registry, {Vibe.Registry, {:lsp, Path.expand(cwd)}}}
end
