defmodule Vibe.Tool.Builtin.Eval do
  @moduledoc "Model-facing Elixir eval tool."
  import JSONSpec

  alias Vibe.Tool.AdapterResult, as: ToolResult
  ...
  use Jido.Action,
    name: "eval",
    description:
      "Evaluate Elixir code inside Vibe's BEAM runtime. Prefer this for OTP introspection, profiling, docs, and helper modules.",
    schema: @schema
  ...
  def run(params, context) do
    ...
    Vibe.Tool.PluginHooks.run(:eval, params, context, fn params ->
      ...
      |> case do
        {:ok, result} -> ToolResult.ok(Vibe.Eval.Result.to_tool_output(result))
  ...
  defp evaluate(code, session_id, opts) when is_binary(session_id),
    do: Vibe.Eval.run(code, Keyword.put(opts, :session_id, session_id))

  defp evaluate(code, _session_id, opts), do: Vibe.Eval.once(code, opts)
