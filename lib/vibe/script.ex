defmodule Vibe.Script do
  ...
  def run_string(source, opts \\ []) when is_binary(source) do
    tmp_dir = Path.join(System.tmp_dir!(), "vibe-script-#{System.unique_integer([:positive])}")
  ...
  defp run_os_process(path, opts) do
    ...
    args = Vibe.Support.Lists.append(Keyword.get(opts, :args, []), path)
    cwd = Keyword.get(opts, :cwd, File.cwd!())
    env = Keyword.get(opts, :env, [])
    timeout = Keyword.get(opts, :timeout, @default_timeout_ms)
  ...
  defp run_port(executable, args, opts) do
    port =
      Port.open({:spawn_executable, executable}, [
        ...
        env: Vibe.Env.to_charlist_pairs(Keyword.fetch!(opts, :env))
      ])
