defmodule Vibe.Gateway.Telegram.Config do
  @moduledoc """
  ...
  defstruct token: nil,
            ...
            poll_timeout_s: 5,
            poll_receive_timeout_ms: 10_000,
            poll_max_consecutive_conflicts: 12
  ...
  @type t :: %__MODULE__{
          ...
          poll_receive_timeout_ms: pos_integer(),
          poll_max_consecutive_conflicts: pos_integer()
        }
    ...
    with {:ok, token} <- fetch_token(overrides),
      ...
      {:ok,
       %__MODULE__{
         ...
         poll_timeout_s:
           integer_setting(overrides, :poll_timeout_s, "TELEGRAM_POLL_TIMEOUT_S", 5),
         poll_receive_timeout_ms:
           integer_setting(
             overrides,
             :poll_receive_timeout_ms,
             "TELEGRAM_POLL_RECEIVE_TIMEOUT_MS",
             10_000
           ),
         poll_max_consecutive_conflicts:
           integer_setting(
             overrides,
             :poll_max_consecutive_conflicts,
             "TELEGRAM_POLL_MAX_CONSECUTIVE_CONFLICTS",
             12
           )
       }}
    end
  ...
  defp setting(overrides, key, env) do
    Keyword.get(overrides, key) ||
      Application.get_env(:vibe, telegram_env_key(key)) ||
      System.get_env(env)
  end
