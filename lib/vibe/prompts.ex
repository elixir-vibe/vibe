defmodule Vibe.Prompts do
  ...
  for {_name, file} <- @prompt_files do
    @external_resource Application.app_dir(:vibe, Path.join("priv/prompts", file))
  end

  @prompts Map.new(@prompt_files, fn {name, file} ->
             {name, File.read!(Application.app_dir(:vibe, Path.join("priv/prompts", file)))}
           end)
  ...
  def system, do: Application.get_env(:vibe, :system_prompt, fetch!(:system))
