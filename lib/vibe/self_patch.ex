defmodule Vibe.SelfPatch do
  ...
  def validate(opts \\ []) do
    report = Vibe.Code.Checks.analyze(opts)
  ...
  def compile_and_reload(opts \\ []) do
    ...
    with {:ok, check_report} <- validate(opts),
      ...
      Vibe.Session.Store.append_trajectory(:self_patch_reloaded, report)
      {:ok, report}
    end
  end
  ...
  def release_reload(opts \\ []) do
    vsn = opts |> Keyword.get(:vsn, Application.spec(:vibe, :vsn)) |> to_charlist()
  ...
  defp reloadable_modules([], _opts), do: vibe_modules()
  ...
  defp reloadable_module?(module, opts) do
    prefixes = Keyword.get(opts, :module_prefixes, ["Elixir.Vibe"])
    Enum.any?(prefixes, &(module |> to_string() |> String.starts_with?(&1)))
  end

  defp vibe_modules do
    :vibe
