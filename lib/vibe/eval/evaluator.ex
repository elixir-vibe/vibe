defmodule Vibe.Eval.Evaluator do
  ...
  defp eval_with_captured_io(code, state) do
    {{success?, result, state}, io} =
      capture_io(fn ->
        Vibe.Command.Streaming.with_eval_session(state.session_id, fn ->
          eval_code(code, state)
        end)
      end)
  ...
  defp via(session_id), do: {:via, Registry, {Vibe.Registry, {:eval, session_id}}}
end
