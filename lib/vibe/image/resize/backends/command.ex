defmodule Vibe.Image.Resize.Backends.Command do
      ...
      {:ok, binary} ->
        root =
          Path.join(
            ...
            "vibe-image-resize-#{System.unique_integer([:positive])}"
          )
  ...
  def run(argv, opts \\ []) do
    case Command.run(argv, Keyword.put_new(opts, :timeout, 30_000)) do
      %Vibe.Command.Result{status: :ok} -> :ok
      %Vibe.Command.Result{} = result -> {:error, {:command_failed, result.status, result.output}}
