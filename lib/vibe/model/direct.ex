defmodule Vibe.Model.Direct do
  ...
  def stream(prompt, opts \\ []) when is_binary(prompt) or is_list(prompt) do
    ...
    with {:ok, request_opts} <-
           Vibe.Model.Transport.prepare_stream_opts(model, request_opts, session_id),
  ...
  defp record_response({:ok, response}, session_id) do
    Vibe.Session.Store.append_trajectory(:assistant_message, %{result: response},
      session_id: session_id
    )

    if usage = Vibe.Model.Usage.from_response(response) do
      Vibe.Session.Store.append_trajectory(:llm_usage, usage, session_id: session_id)
    end
  end

  defp record_response({:error, reason}, session_id) do
    Vibe.Session.Store.append_trajectory(:assistant_message, %{error: inspect(reason)},
      session_id: session_id
    )
  end
end
