defmodule Vibe.Model.Transport do
  @moduledoc """
  ...
  boundary that turns Vibe/profile transport policy into ReqLLM provider options
  ...
  defp put_reusable_responses_websocket(model, request_opts, provider_opts, session_id) do
    with {:ok, pid} <- Vibe.Model.Transport.WebSocketPool.get(model, request_opts, session_id) do
      provider_opts =
        provider_opts
