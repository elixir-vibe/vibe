defmodule Exy.Gateway do
  @moduledoc """
  Introspection helpers for external gateway runtimes.

  Gateway workers are still started by `Exy.Gateway.Supervisor`; this module
  ...
  @spec statuses() :: [map()]
  def statuses do
    configured = Application.get_env(:exy, :gateways, [])
  ...
  @doc "Returns recent Exy sessions created by gateways."
  ...
  defp ensure_known_telegram_status(statuses) do
    if Enum.any?(statuses, &(&1.id == :telegram)) do
      ...
      [runtime_status(:telegram, Exy.Gateway.Telegram.Backend, nil) | statuses]
    end
  ...
  defp runtime_status(id, backend, config) do
    runtime = {:via, Registry, {Exy.Registry, {:gateway_runtime, id}}}

    case Registry.lookup(Exy.Registry, {:gateway_runtime, id}) do
      [{pid, _value}] ->
        %{
          id: id,
          backend: backend,
          pid: inspect(pid),
          status: :running,
          stats: Exy.Gateway.Runtime.stats(runtime),
