defmodule Exy.Gateway.Telegram.StreamConsumer do
  @moduledoc """
  ...
  in the generic `Exy.Gateway.StreamConsumer`; this module exists for `:draft`
  ...
  def handle_cast({:delta, text}, state) do
    state = %{
      state
      | accumulated: state.accumulated <> Exy.Gateway.StreamConsumer.filter_display_text(text)
    }
