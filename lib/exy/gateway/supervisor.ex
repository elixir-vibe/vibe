defmodule Exy.Gateway.Supervisor do
  @moduledoc """
  Supervisor for configured external gateway runtimes.

  Each gateway runs the generic `Exy.Gateway.Runtime` with a backend module and
        ...
        {Runtime, :start_link,
         [
           [
             backend: backend,
             config: config,
             dispatch_fun: Keyword.get(opts, :dispatch_fun, &Exy.Gateway.Dispatcher.dispatch/2),
