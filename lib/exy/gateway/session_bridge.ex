defmodule Exy.Gateway.SessionBridge do
  @moduledoc """
  Bridges Exy session assistant stream events back to an external gateway.

  A bridge is created per accepted gateway message. It subscribes to the target
  Exy session and forwards only assistant-facing stream events to a
  `Exy.Gateway.StreamConsumer`, keeping Telegram/Slack/etc. delivery outside
  the session and agent internals.
  """

  use GenServer

  alias Exy.Gateway.{Message, StreamConsumer}
  alias Exy.UI.Event

  @spec start(Message.t(), String.t(), keyword()) :: {:ok, pid()} | {:error, term()}
  def start(%Message{} = message, session_id, opts \\ []) do
    DynamicSupervisor.start_child(
      Exy.Gateway.BridgeSupervisor,
      %{
  ...
  def init(opts) do
    ...
    with {:ok, session} <- Exy.Session.lookup(session_id),
         :ok <- Exy.Session.subscribe(session, self()) do
      {:ok,
       %{
         session: session,
  ...
  @impl true
  def handle_info({Exy.Session, :event, %Event{} = event}, state) do
