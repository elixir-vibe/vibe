defmodule Exy.Memory.Manager do
  ...
  def handle_call({:load, module, opts}, _from, state) do
    external? = module != Exy.Memory.BuiltinProvider
  ...
  defp configured_providers do
    [Exy.Memory.BuiltinProvider | Application.get_env(:exy, :memory_providers, [])]
    |> Enum.uniq()
  end
  ...
  defp start_provider(module, opts) do
    with {:ok, provider_state} <- module.init(opts) do
      {:ok,
       %{module: module, state: provider_state, external?: module != Exy.Memory.BuiltinProvider}}
    end
  rescue
