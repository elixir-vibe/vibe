...
- Store durable runtime state in local SQLite through Ecto schemas, migrations, and `Exy.Repo`; do not add app-level raw SQL storage helpers. Use `Exy.Storage.migrate!/0`, `Exy.Storage.status/0`, and storage CLI commands for schema lifecycle.
