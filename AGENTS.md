...
- Plugin-owned UI must be semantic and renderer-neutral: expose plugin detail/inspector UI as `Exy.UI.Document` / `Exy.UI.Section` containing `Exy.UI.Widget` values, and use `Exy.Plugin.UI` status/widget APIs for live session UI. Do not introduce plugin-specific web pages, HEEx/HTML/Tailwind/ANSI contracts, or parallel UI structs under plugin modules.
...
- Domain structs that cross a JSON boundary should define their `Jason.Encoder` implementation next to the struct module. Keep shared JSON value normalization in `Exy.JSON.Encode`; do not create broad centralized encoder files for domain structs or hand-roll JSON codecs with static event-type maps when `Jason` plus typed boundary decoders can do the job.
...
- Subagents are supervised jobs with child sessions. Use `Exy.Subagents.start/2`, `ask/2`, `run_many/2`, and `schedule/2`; attach to a running child session with `exy a <child_session_id>`. Persist job/schedule state through `Exy.Subagents.JobStore` / `Exy.Subagents.Store`, not JSONL files.
- Model/role preferences live in editable TOML via `Exy.Agent.Profile`. OpenRouter is a regular provider, not a special architecture path.
- Model and effort switching are session-level semantic commands and TUI keybindings first, not slash-command-owned features. Keep `/model` and `/effort` as thin aliases over `Exy.Session` commands. Internal effort values follow Pi-style atoms: `:off`, `:minimal`, `:low`, `:medium`, `:high`, `:xhigh`; parse strings only at UI/config boundaries.
...
- For storage imports, add an `Exy.Storage.Importer` implementation and register it in `Exy.Storage.Import`; keep provider-specific parsing (such as Pi JSONL) out of the dispatcher. Preserve source metadata such as cwd and distinguish conversation text from imported tool output for search quality.
