defmodule Vibe.TUI.TerminalLoopTest do
    ...
    :ok =
      Vibe.Session.emit_transient_event(
        ...
        Vibe.Event.new(
          :tool_started,
          session_id,
          tool_payload_started(id: "read-1", name: :read, args: %{path: "large.ex"})
        )
      )
  ...
  test "editor-only input reuses cached body blocks" do
    ...
    first = TerminalLoop.render_frame(loop, :full)
    :ok = TerminalLoop.input(loop, "x")
    second = TerminalLoop.render_frame(loop, :full)
    ...
    assert :ok =
             Vibe.Session.emit_transient_event(
               ...
               Vibe.Event.new(
                 :tool_started,
                 session_id,
                 tool_payload_started(id: "eval-1", name: :eval, args: %{code: code})
               )
             )
  ...
  test "loader says working while local tool work is running" do
    ...
    assert :ok =
             Vibe.Event.Bus.emit(
               session_id,
               :tool_started,
               tool_payload_started(id: "eval-1", name: :eval)
             )
  ...
  defp tool_started(session_id, id, name, args) do
    Vibe.Event.new(
      :tool_started,
      session_id,
      tool_payload_started(id: id, name: name, args: args)
    )
  end
