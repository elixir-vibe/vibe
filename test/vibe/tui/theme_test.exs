defmodule Vibe.TUI.ThemeTest do
  use ExUnit.Case, async: false

  test "provides dark and light themes" do
    assert Vibe.TUI.Theme.dark().name == "dark"
    assert Vibe.TUI.Theme.light().name == "light"
    assert Vibe.TUI.Theme.named("light").name == "light"
  end

  test "auto-detects light and dark terminal backgrounds" do
    with_env(%{"VIBE_THEME" => nil, "COLORFGBG" => "15;0"}, fn ->
      assert Vibe.TUI.Theme.default().name == "dark"
    end)

    with_env(%{"VIBE_THEME" => nil, "COLORFGBG" => "0;15"}, fn ->
      assert Vibe.TUI.Theme.default().name == "light"
    end)
  end
  ...
  test "applies semantic foreground colors with ANSI reset" do
    styled = Vibe.TUI.Theme.default() |> Vibe.TUI.Theme.fg(:error, "boom")

    styled = IO.iodata_to_binary(styled)

    assert styled =~ "\e[38;2;204;102;102m"
    assert styled =~ IO.ANSI.reset()
    assert Vibe.TUI.Theme.strip(styled) == "boom"
  end

  test "applies RGB backgrounds through truecolor ANSI" do
    styled = Vibe.TUI.Theme.default() |> Vibe.TUI.Theme.bg(:tool_pending_bg, "tool")

    styled = IO.iodata_to_binary(styled)

    assert styled =~ "\e[48;2;34;36;42m"
    assert Vibe.TUI.Theme.strip(styled) == "tool"
  end
