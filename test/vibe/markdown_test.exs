defmodule Vibe.MarkdownTest do
  use ExUnit.Case, async: true

  alias Vibe.Command.Result
  alias Vibe.Plugins.WebSearch.Result, as: WebSearchResult

  test "renders command results" do
    markdown =
      Vibe.Markdown.to_markdown(%Result{
        id: "cmd-1",
        argv: ["mix", "test"],
        cwd: "/tmp/project",
        status: :ok,
        ...
        output_path: "/tmp/project/.vibe/commands/cmd-1.log",
        duration_ms: 12
      })
  ...
  test "MD alias is available in eval" do
    assert {:ok, result} =
             Vibe.Eval.run(~S|MD.to_markdown(%{ok: true})|, session_id: "md-alias-test")

    assert result.output =~ "ok: `true`"
  end

  test "MD doc marks eval output as markdown" do
    assert {:ok, result} = Vibe.Eval.run(~S|MD.doc(%{ok: true})|, session_id: "md-doc-test")
