defmodule Vibe.UI.BusTest do
  ...
  test "broadcast skips stale session registrations" do
    ...
    assert :ok = Vibe.UI.Bus.notify_all(%{level: :warning, text: "runtime pressure"})
    assert {:error, :not_found} = Vibe.UI.Bus.server(session_id)
  end
  ...
  test "replacing a session demonitor old server and keeps the replacement" do
    ...
    assert :ok = Vibe.UI.Bus.register(session_id, old)
    assert :ok = Vibe.UI.Bus.register(session_id, replacement)
    assert {:ok, ^replacement} = Vibe.UI.Bus.server(session_id)
    ...
    assert :ok = Vibe.UI.Bus.unregister(session_id, replacement)
    assert {:error, :not_found} = Vibe.UI.Bus.server(session_id)
