defmodule Vibe.Auth.OpenCodeTest do
  use ExUnit.Case, async: false

  alias Vibe.Auth.OpenCode

  setup do
    previous = Application.get_env(:vibe, :opencode_credentials)

    on_exit(fn ->
      if previous,
        do: Application.put_env(:vibe, :opencode_credentials, previous),
        else: Application.delete_env(:vibe, :opencode_credentials)
    end)
  ...
  test "api_key returns nil when not configured" do
    Application.delete_env(:vibe, :opencode_credentials)
  ...
  test "load falls back to OPENCODE_API_KEY env var" do
    ...
    tmp = Path.join(System.tmp_dir!(), "vibe-opencode-test-#{System.unique_integer([:positive])}")
    Application.put_env(:vibe, :home_dir, tmp)
