defmodule Vibe.Model.Transport.WebSocketPoolTest do
  ...
  test "rejects providers without reusable Responses WebSocket support" do
    ...
    assert {:error, {:unsupported_reusable_websocket_provider, :openrouter}} =
             Vibe.Model.Transport.WebSocketPool.get(model, [], "session-websocket-test")
  end

  test "closes cached sessions for a session id" do
    assert :ok = Vibe.Model.Transport.WebSocketPool.close_session("session-websocket-test")
  end
end
