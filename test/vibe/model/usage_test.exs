defmodule Vibe.Model.UsageTest do
  use ExUnit.Case, async: true

  test "extracts and summarizes model response usage" do
    usage =
      Vibe.Model.Usage.from_response(%{
    ...
    assert Vibe.Model.Usage.summarize([usage]).total_tokens == 10
  end
  ...
  test "summarizes total tokens from input and output when provider omits total" do
    assert %{total_tokens: 10} =
             Vibe.Model.Usage.summarize([%{input_tokens: 4, output_tokens: 6}])
  end
