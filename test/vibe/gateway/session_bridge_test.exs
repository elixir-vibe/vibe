defmodule Vibe.Gateway.SessionBridgeTest do
  ...
  test "streams assistant deltas to the gateway adapter" do
    ...
    Vibe.Session.emit_event(session, Event.new(:assistant_stream_started, session_id, %{}))
    Vibe.Session.emit_event(session, Event.new(:assistant_delta, session_id, %{text: "hi"}))

    assert_receive {:gateway_send, "chat-1", "hi ▉", opts}
    assert opts[:reply_to] == "reply-1"

    Vibe.Session.emit_event(session, Event.new(:assistant_delta, session_id, %{text: " there"}))
