defmodule Exy.Actions.ReadImageToolResultTest do
  ...
  test "image read results expose transient ReqLLM content parts for ReAct turns" do
    ...
    dir =
      Path.join(System.tmp_dir!(), "exy-read-image-tool-#{System.unique_integer([:positive])}")
    ...
    try do
      ...
      assert [
               %ReqLLM.Message.ContentPart{type: :text},
               %ReqLLM.Message.ContentPart{type: :image_url} = image
      ...
      assert [
               %ReqLLM.Message.ContentPart{type: :text},
               %ReqLLM.Message.ContentPart{type: :text},
               %ReqLLM.Message.ContentPart{type: :image_url} = formatted_image
             ] = formatted
  ...
  test "transient ReqLLM content parts are not duplicated in JSON storage" do
    result = %Exy.Files.ReadResult{
      path: "tiny.png",
      content_type: :image,
      parts: [Exy.Model.Content.text("Read image")],
      __content_parts__: [ReqLLM.Message.ContentPart.text("Read image")]
    }
