defmodule Exy.Auth.CodexTest do
  ...
  setup do
    ...
    home =
      Path.join(System.tmp_dir!(), "exy-codex-auth-test-#{System.unique_integer([:positive])}")
  ...
  test "auth store persists atom-keyed oauth credentials as JSON-safe data" do
    ...
    assert :ok = Exy.Auth.Store.save("openai-codex", credentials)
    assert {:ok, loaded} = Exy.Auth.Store.load("openai-codex")
  ...
  test "login prints failure reason" do
    output =
      capture_io(:stderr, fn ->
        assert {:error, _reason} = Exy.Auth.Codex.login(open_browser: false, timeout: 1)
      end)
