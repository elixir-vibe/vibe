...
```elixir
Vibe.Plugin.UI.set_status(session_id, :indexer, "indexing")
Vibe.Plugin.UI.set_progress(session_id, :indexer, title: "Indexing", current: 1, total: 10)
Vibe.Plugin.UI.set_widget(session_id, :panel, ["line 1", "line 2"], placement: :below_editor)
```

Plugins can render their own structs by implementing `Vibe.Markdown`, or by exposing `to_markdown/1` on runtime-loaded structs handled by the fallback renderer.
